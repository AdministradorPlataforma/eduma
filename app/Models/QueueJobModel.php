...
class QueueJobModel extends BaseModel {
    ...
    // Si BaseModel tiene create/update methods genericos, los usamos.
    // Si no, definimos aqui. 
    // Asumire que BaseModel provee metodos basicos tipo Eloquent o Query Builder simple. 
    ...
    public function getNextPending(int $maxAttempts = 3) {
        ...
        // Buscamos uno que esté 'pending' o 'failed' (reintento) y no esté reservado
    ...
    public function getRecentJobs(int $limit = 10): array {
        $sql = "SELECT id, queue, status, attempts, created_at, processed_at, last_error FROM {$this->table} ORDER BY id DESC LIMIT " . (int)$limit;
